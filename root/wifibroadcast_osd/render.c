...
//	TextEnd(getWidth(94), getHeight(80), "ᐛ", osdicons, text_scale*2.3);	
    ...
    #else
    case FLTMODE_COPTER_STABILIZE: sprintf(buffer, "STAB"); break;
    case FLTMODE_COPTER_ACRO: sprintf(buffer, "ACRO"); break;
    case FLTMODE_COPTER_ALTHOLD: sprintf(buffer, "ALTHOLD"); break;
    case FLTMODE_COPTER_AUTO: sprintf(buffer, "AUTO"); break;
    case FLTMODE_COPTER_GUIDED: sprintf(buffer, "GUIDED"); break;
    case FLTMODE_COPTER_LOITER: sprintf(buffer, "LOITER"); break;
    case FLTMODE_COPTER_RTL: sprintf(buffer, "RTL"); break;
    case FLTMODE_COPTER_CIRCLE: sprintf(buffer, "CIRCLE"); break;
    case FLTMODE_COPTER_LAND: sprintf(buffer, "LAND"); break;
    case FLTMODE_COPTER_DRIFT: sprintf(buffer, "DRIFT"); break;
    case FLTMODE_COPTER_SPORT: sprintf(buffer, "SPORT"); break;
    case FLTMODE_COPTER_FLIP: sprintf(buffer, "FLIP"); break;
    case FLTMODE_COPTER_AUTOTUNE: sprintf(buffer, "AUTOTUNE"); break;
    case FLTMODE_COPTER_POSHOLD: sprintf(buffer, "POSHOLD"); break;
    case FLTMODE_COPTER_BRAKE: sprintf(buffer, "BRAKE"); break;
    case FLTMODE_COPTER_THROW: sprintf(buffer, "THROW"); break;
    case FLTMODE_COPTER_AVOID_ADSB: sprintf(buffer, "AVOIDADSB"); break;
    case FLTMODE_COPTER_GUIDED_NO_GPS: sprintf(buffer, "GUIDEDNOGPS"); break;

    case FLTMODE_COPTER_SMART_RTL: sprintf(buffer, "SMART RTL"); break;
    case FLTMODE_COPTER_FLOW_HOLD: sprintf(buffer, "FLOW HOLD"); break;
    case FLTMODE_COPTER_FOLLOW: sprintf(buffer, "FOLLOW"); break;
    case FLTMODE_COPTER_ZIGZAG: sprintf(buffer, "ZigZag"); break;
    case FLTMODE_COPTER_SYSTEM_ID: sprintf(buffer, "SystemId"); break;
    case FLTMODE_COPTER_HELI_AUTOROTATE: sprintf(buffer, "AUTOROTATE"); break;
    case FLTMODE_COPTER_AUTO_RTL: sprintf(buffer, "AUTO RTL"); break;
    case 255: sprintf(buffer, "-----"); break;
    #endif
    #else       
    #if CHINESE == true
      ...
      case 255: sprintf(buffer, "-----"); break;
    #else
      case FLTMODE_PLANE_MANUAL: sprintf(buffer, "MAN"); break;
      case FLTMODE_PLANE_CIRCLE: sprintf(buffer, "CIRC"); break;
      case FLTMODE_PLANE_STABILIZ: sprintf(buffer, "STAB"); break;
      case FLTMODE_PLANE_TRAINING: sprintf(buffer, "TRAI"); break;
      case FLTMODE_PLANE_ACRO: sprintf(buffer, "ACRO"); break;
      case FLTMODE_PLANE_FBWA: sprintf(buffer, "FBWA"); break;
      case FLTMODE_PLANE_FBWB: sprintf(buffer, "FBWB"); break;
      case FLTMODE_PLANE_CRUISE: sprintf(buffer, "CRUZ"); break;
      case FLTMODE_PLANE_AUTOTUNE: sprintf(buffer, "TUNE"); break;
      case FLTMODE_PLANE_AUTO: sprintf(buffer, "AUTO"); break;
      case FLTMODE_PLANE_RTL: sprintf(buffer, "RTL"); break;
      case FLTMODE_PLANE_LOITER: sprintf(buffer, "LOIT"); break;
      case FLTMODE_PLANE_TAKEOFF: sprintf(buffer, "TKOFF"); break;
      case FLTMODE_PLANE_AVOID_ADSB: sprintf(buffer, "ADSB"); break;
      case FLTMODE_PLANE_GUIDED: sprintf(buffer, "GUID"); break;
      case 16: sprintf(buffer, "INIT"); break;
      case FLTMODE_PLANE_QSTABILIZE: sprintf(buffer, "Q-STAB"); break;
      case FLTMODE_PLANE_QHOVER: sprintf(buffer, "Q-HOVR"); break;
      case FLTMODE_PLANE_QLOITER: sprintf(buffer, "Q-LOIT"); break;
      case FLTMODE_PLANE_QLAND: sprintf(buffer, "Q-LAND"); break;
      case FLTMODE_PLANE_QRTL: sprintf(buffer, "Q-RTL"); break;
      case FLTMODE_PLANE_QAUTOTUNE: sprintf(buffer, "Q-TUNE"); break;
      case FLTMODE_PLANE_QACRO: sprintf(buffer, "Q-ACRO"); break;
      case FLTMODE_PLANE_THERMAL: sprintf(buffer, "THERM"); break;
      case FLTMODE_PLANE_LOITER_TO_QLAND: sprintf(buffer, "QLAND-LT"); break;
      case 255: sprintf(buffer, "-----"); break;
    #endif
...
void draw_alt_ladder(telemetry_data_t *td, float pos_x, float pos_y, float scale, float warn, float caution, float vsi_time, float climb){
    ...
    if(td->terrain_data.spacing != 0) //no terrain data avalible
    {
...
void draw_ahi_mav(telemetry_data_t *td, float roll, float pitch, float climb, float vz, float vx, float vy, float gpsspeed, float alt, float scale)
{
    ...
    while (k <= max)
    {
        ...
        // HOME ICON
            ...
            Fill(255, 128, 0, getOpacity(COLOR)); // Orange
            float home_heading = course_to(td->latitude, td->longitude, td->home_lat, td->home_lon);
            float home_deviation = home_heading - td->heading;
            if (home_deviation > 180)
                home_deviation -= 360;
            if (home_deviation < -180)
                home_deviation += 360;
...
// Rollangle on main line
    ...
    Stroke(COLOR);
    StrokeWidth(3);
    CircleOutline(pos_x, pos_y, getWidth(2)*scale);
    StrokeWidth(OUTLINEWIDTH);
    Stroke(OUTLINECOLOR);
    */
